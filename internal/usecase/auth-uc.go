...
type AuhtUc interface {
	Login(ctx context.Context, payload string) (dto.AuthResponseDto, error)
...
func (u *authUc) Login(ctx context.Context, payload string) (dto.AuthResponseDto, error) {
	u.log.Info("login in usecase", payload)
	user, err := u.authRepo.Login(ctx, payload)
	if err != nil {
...
func (u *authUc) Register(ctx context.Context, payload dto.RegisterReq) error {
	...
	u.log.Info("hashing password", payload.Password)
